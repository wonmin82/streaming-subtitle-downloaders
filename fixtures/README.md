...
The verifier is dependency-free and offline. It rejects malformed schema versions, path traversal, symbolic links, unreferenced inputs, unreviewed expectations, and truncated captures, and runs safety checks for sensitive content. Repository verification has a [known input-body scanning gap](../docs/known-limitations.md#fixture-safety-and-fidelity), so a successful `verify` or `verify-all` result is not proof that every input file is free of credentials or signed URLs. Review each referenced input directly. Passing `verify-capture` means a local export cleared the capture schema and high-risk-data checks; it does not mean detected titles, language labels, status text, or output filenames have been anonymized.
...
For XML inputs, also review the [capture sanitizer's identifier/reference and namespace limitation](../docs/known-limitations.md#fixture-safety-and-fidelity). Sanitized TTML can otherwise replay different styling or layout from the original input.
