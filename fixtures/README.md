...
`observed.json` is evidence, not an oracle. Import intentionally writes `"reviewed": false` and empty assertions. A maintainer must diagnose the behavior, write the intended result in `expected.json`, and set `reviewed` to `true`. Unreviewed fixtures fail repository verification.
