...
Session values and opaque identifiers are replaced with stable capture-local aliases. URL credentials, query signing parameters, and path-embedded CDN signing segments are removed. Structure-only metadata artifacts replace private or copyright-bearing text with placeholders, account/profile fields are removed, and subtitle dialogue is replaced while supported timing and structure are retained. XML identifier references and namespace values are not always preserved correctly; review the [fixture fidelity limitation](known-limitations.md#fixture-safety-and-fidelity) before using an XML capture as a replay input. Capture sizes and entry counts are bounded; an export that hits a limit is marked as truncated and cannot be imported as a repository fixture.
...
Repository verification checks the schema, reviewed flag, paths, symlinks, referenced inputs, and size limits, and runs safety checks for sensitive content. However, the generic secret and signed-URL scan does not cover every referenced input body; see the [known scanning gap](known-limitations.md#fixture-safety-and-fidelity). Inspect all input files directly even when `verify` or `verify-all` passes. Replay tests then feed fixture inputs into the applicable userscript parser and compare the fresh result with the human-reviewed assertions. No streaming-service request is allowed during replay.
